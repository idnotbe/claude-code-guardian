...
try:
    ...
    print(f"Warning: Could not import protection utils: {e}", file=sys.stderr)
    sys.exit(0)  # Continue session termination


def main():
    """Execute auto-commit on session stop."""
    log_protection("INFO", "auto-commit hook triggered (Stop event)")

    # Check circuit breaker - skip commit if circuit is open
    circuit_open, reason = is_circuit_open()
    if circuit_open:
        log_protection("WARN", f"Circuit breaker is OPEN - skipping auto-commit: {reason}")
        # PLUGIN MIGRATION: Updated path reference
        log_protection("INFO", "To resume auto-commits, delete .claude/guardian/.circuit_open")
        return
    ...
    if not git_integration:
        log_protection("WARN", "gitIntegration section missing from protection.json")
        return
    ...
    if not git_config.get("enabled", False):
        log_protection("INFO", "Auto-commit disabled (gitIntegration.autoCommit.enabled=false)")
        return

    if not git_config.get("onStop", False):
        log_protection("INFO", "Auto-commit on stop disabled")
        return
    ...
    if is_rebase_or_merge_in_progress():
        log_protection("WARN", "Rebase/merge in progress - skipping auto-commit")
        return
    ...
    # Dry-run mode
    if is_dry_run():
        log_protection("DRY-RUN", "Would auto-commit changes")
        return
    ...
    if not success:
        log_protection("WARN", "Failed to stage changes, attempting commit anyway")
        ...
        log_protection("INFO", f"Staged {mode}")
    ...
    # P1-2 FIX: Enforce message length limit for consistency with bash_protection.py
    ...
    if not git_has_staged_changes():
        log_protection("INFO", "No staged changes to commit - skipping (this is normal)")
    ...
    if git_commit(message, no_verify=True):
        ...
        if commit_hash:
            log_protection("INFO", f"auto-commit success: {commit_hash} - {message}")
        else:
            log_protection("INFO", f"auto-commit success: (hash unavailable) - {message}")
        ...
        log_protection("WARN", "Auto-commit failed - check earlier warnings for details")
...
if __name__ == "__main__":
    try:
        ...
        log_protection("ERROR", f"Auto-commit hook error: {e}")
